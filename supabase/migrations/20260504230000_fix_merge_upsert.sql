-- Add ON CONFLICT fallback to merge_session so that if the time-overlap
-- search fails to find an existing session, we upsert by ID instead of
-- crashing with a duplicate-key 409.
...
begin
    ...
    if found then
        ...
        -- Upsert: insert new session, or merge by ID if it already exists
        -- (handles re-push of active sessions whose time range no longer overlaps
        --  after a previous merge expanded the existing row)
        ...
        on conflict (id) do update set
            ...
            active_duration = sessions.active_duration + excluded.active_duration,
            lines_added     = sessions.lines_added + excluded.lines_added,
            lines_removed   = sessions.lines_removed + excluded.lines_removed,
