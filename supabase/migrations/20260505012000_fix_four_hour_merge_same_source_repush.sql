-- Fix 4-hour merging for repeated pushes from the same source.
--
-- The previous function had a stale-component cleanup that deleted legitimate
-- same-source sessions inside the merge window when an older source session was
-- re-pushed. Keep exact source-session re-pushes idempotent, but never delete a
-- different source_session_id just because it is nearby.
...
begin
    ...
    -- in whatever aggregate session it already belongs to.
    ...
    -- New component: merge into the closest overlapping/nearby aggregate row.
    -- Prefer an existing neighboring aggregate over a same-id orphan row, so
    -- rows created before component backfill can be folded into the canonical
    -- session instead of preserving a split card.
...
-- Recreate missing components from visible session rows, then consolidate all
-- component-backed sessions into 4-hour groups by repo/branch.
insert into session_components (
    user_id, session_id, source, source_session_id,
    started_at, ended_at, active_duration,
    lines_added, lines_removed, files_touched, tokens, model
